...
- (UIImage *)parseChannel:(NSString *)channel {
    
    if ([channel isEqualToString:PayChannelWxApp]) {
        return [self imagesNamedFromCustomBundle:@"wx"];
    }
    if ([channel isEqualToString:PayChannelAliApp]) {
        return [self imagesNamedFromCustomBundle:@"ali"];
    }
    if ([channel isEqualToString:PayChannelUnApp]) {
        return [self imagesNamedFromCustomBundle:@"un"];
    }
    if ([channel isEqualToString:PayChannelBaiduApp] || [channel isEqualToString:PayChannelBaiduWap]) {
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    dic[kKeyResponseResultCode] = @(resultCode);
    dic[kKeyResponseResultMsg] = resultMsg;
    dic[kKeyResponseErrDetail] = resultMsg;
    
    if ([BCPaySDK getBeeCloudDelegate] && [[BCPaySDK getBeeCloudDelegate] respondsToSelector:@selector(onBCPayResp:)]) {
        [[BCPaySDK getBeeCloudDelegate] onBCPayResp:dic];
    }
}
...
- (void)doNotifyResponse:(NSDictionary *)response {
    [loading stopAnimating];
    if ([response integerValueForKey:kKeyResponseResultCode defaultValue:BCErrCodeCommon] == BCSuccess) {
        resultMsg = @"支付成功";
...
- (void)doErrorResponse:(NSString *)msg errDetail:(NSString *)errMsg {
    NSMutableDictionary *dic =[NSMutableDictionary dictionaryWithCapacity:10];
    dic[kKeyResponseResultCode] = @(BCErrCodeCommon);
    dic[kKeyResponseResultMsg] = msg;
